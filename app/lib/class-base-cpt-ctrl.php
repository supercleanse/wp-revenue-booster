...
abstract class Base_Cpt_Ctrl extends Base_Ctrl {
  public $cpt, $ctaxes;
  ...
  public function post_updated_messages( $messages ) {
    ...
    if($public) {
      $messages[$slug][1] = sprintf( __('%1$s updated. <a href="%2$s">View %3$s</a>', 'wp-revenue-booster'),
                                     $singular_name, esc_url( get_permalink($post_ID) ), $singular_name );
    }
    else {
      $messages[$slug][1] = sprintf( __('%1$s updated.', 'wp-revenue-booster'), $singular_name );
    }

    $messages[$slug][2] = __('Custom field updated.', 'wp-revenue-booster');
    $messages[$slug][3] = __('Custom field deleted.', 'wp-revenue-booster');
    $messages[$slug][4] = sprintf( __('%s updated.', 'wp-revenue-booster'), $singular_name );
    $messages[$slug][5] = isset($_GET['revision']) ? sprintf( __('%1$s restored to revision from %2$s', 'wp-revenue-booster'), $singular_name, wp_post_revision_title( (int) $_GET['revision'], false ) ) : false;

    if($public) {
      $messages[$slug][6] = sprintf( __('%1$s published. <a href="%2$s">View %3$s</a>', 'wp-revenue-booster'),
                                     $singular_name, esc_url( get_permalink($post_ID) ), $singular_name );
    }
    else {
      $messages[$slug][6] = sprintf( __('%1$s published.', 'wp-revenue-booster'), $singular_name );
    }

    $messages[$slug][7] = sprintf( __('%s saved.', 'wp-revenue-booster'), $singular_name );

    if($public) {
      $messages[$slug][8] = sprintf( __('%1$s submitted. <a target="_blank" href="%2$s">Preview %3$s</a>', 'wp-revenue-booster'), $singular_name,
                                     esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ), $singular_name );
      $messages[$slug][9] = sprintf( __('%1$s scheduled for: <strong>%2$s</strong>. <a target="_blank" href="%3$s">Preview %4$s</a>', 'wp-revenue-booster'),
                            $singular_name, date_i18n('M j, Y @ G:i', strtotime($post->post_date), true),
                            esc_url( get_permalink($post_ID) ), $singular_name );
      $messages[$slug][10] = sprintf( __('%1$s draft updated. <a target="_blank" href="%2$s">Preview %3$s</a>', 'wp-revenue-booster'),
                             $singular_name, esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ),
                             $singular_name );
    }
    else {
      $messages[$slug][8] = sprintf( __('%s submitted.', 'wp-revenue-booster'), $singular_name );
      $messages[$slug][9] = sprintf( __('%1$s scheduled for: <strong>%2$s</strong>.', 'wp-revenue-booster'), $singular_name,
                            date_i18n('M j, Y @ G:i', strtotime($post->post_date), true) );
      $messages[$slug][10] = sprintf( __('%s draft updated.', 'wp-revenue-booster'), $singular_name );
    }
  ...
  public function bulk_post_updated_messages( $messages, $counts ) {
    ...
    $messages[$slug] = array(
      'updated'   => _n( sprintf('%1$d %2$s updated.', $counts['updated'], $singular_name),
                         ...
                         $counts['updated'] , 'wp-revenue-booster'),
      'locked'    => _n( sprintf('%1$d %2$s not updated, somebody is editing it.', $counts['locked'], $singular_name),
                         ...
                         $counts['locked'] , 'wp-revenue-booster'),
      'deleted'   => _n( sprintf('%1$d %2$s permanently deleted.', $counts['deleted'], $singular_name),
                         ...
                         $counts['deleted'] , 'wp-revenue-booster'),
      'trashed'   => _n( sprintf('%1$s %2$s moved to the Trash.', $counts['trashed'], $singular_name),
                         ...
                         $counts['trashed'] , 'wp-revenue-booster'),
      'untrashed' => _n( sprintf('%1$s %2$s restored from the Trash.', $counts['untrashed'], $singular_name),
                         ...
                         $counts['untrashed'] , 'wp-revenue-booster')
    );
