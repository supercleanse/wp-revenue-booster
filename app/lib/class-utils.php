...
class Utils {
  ...
  */
  ...
  public static function error_log($error) {
    ...
    error_log(sprintf(__("*** WP Revenue Booster Error\n==========\n%s\n==========\n", 'wp-revenue-booster'),$error));
  }
  ...
  public static function debug_log($message) {
    ...
    if(defined('WP_WPRB_DEBUG') && WP_WPRB_DEBUG) {
      error_log(sprintf(__('*** WP Revenue Booster Debug: %s', 'wp-revenue-booster'),$message));
    }
  }
...
/* PLUGGABLE FUNCTIONS AS TO NOT STEP ON OTHER PLUGINS' CODE */
  ...
  public static function wp_filename($class) {
    return 'class-' . preg_replace("/_/",'-',strtolower($class)) . '.php';
  }

  public static function wp_classname($file_name) {
    $fn = preg_replace('/\.php$/','',basename($file_name));
    $fn = preg_replace('/^class-/','',$fn);
