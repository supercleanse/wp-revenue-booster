...
abstract class Base_Model {
  ...
  protected function validate_not_null($var, $field='') {
    if(is_null($var)) {
      throw new Create_Exception(sprintf(__('%s must not be empty', 'wp-revenue-booster'),$field));
    }
  }
  ...
  protected function validate_is_bool($var, $field='') {
    if(!is_bool($var) && $var!=0 && $var!=1) {
      throw new Create_Exception(sprintf(__('%s must be true or false', 'wp-revenue-booster'),$field));
    }
  }

  protected function validate_is_array($var, $field='') {
    if(!is_array($var)) {
      throw new Create_Exception(sprintf(__('%s must be an array', 'wp-revenue-booster'),$field));
    }
  }

  protected function validate_is_in_array($var, $lookup, $field='') {
    if(is_array($lookup) && !in_array($var, $lookup)) {
      throw new Create_Exception(sprintf(__('%1$s must be %2$s NOT %3$s', 'wp-revenue-booster'),$field, implode( ' '.__('or', 'wp-revenue-booster').' ', $lookup), $var));
    }
  }

  protected function validate_is_url($var, $field='') {
    if(!Utils::is_url($var)) {
      throw new Create_Exception(sprintf(__('%1$s (%2$s) must be a valid url', 'wp-revenue-booster'),$field,$var));
    }
  }

  protected function validate_is_currency($var, $min=0.00, $max=null, $field='') {
    if(!is_numeric($var) || $var < $min || (!is_null($max) && $var > $max)) {
      throw new Create_Exception(sprintf(__('%1$s (%2$s) must be a valid representation of currency', 'wp-revenue-booster'),$field,$var));
    }
  }

  protected function validate_is_numeric($var, $min=0, $max=null, $field='') {
    if(!is_numeric($var) || $var < $min || (!is_null($max) && $var > $max)) {
      throw new Create_Exception(sprintf(__('%1$s (%2$s) must be a valid number', 'wp-revenue-booster'),$field,$var));
    }
  }

  protected function validate_is_email($var, $field='') {
    if(!Utils::is_email($var)) {
      throw new Create_Exception(sprintf(__('%1$s (%2$s) must be a valid email', 'wp-revenue-booster'),$field,$var));
    }
  }

  protected function validate_is_phone($var, $field='') {
    if(!Utils::is_phone($var)) {
      throw new Create_Exception(sprintf(__('%1$s (%2$s) must be a valid phone number', 'wp-revenue-booster'),$field,$var));
    }
  }

  protected function validate_is_ip_addr($var, $field='') {
    if(!Utils::is_ip($var)) {
      throw new Create_Exception(sprintf(__('%1$s (%2$s) must be a valid IP Address', 'wp-revenue-booster'),$field,$var));
    }
  }

  protected function validate_is_date($var, $field='') {
    if(!Utils::is_date($var)) {
      throw new Create_Exception(sprintf(__('%1$s (%2$s) must be a valid date', 'wp-revenue-booster'),$field,$var));
    }
  }
  ...
  protected function validate_is_timestamp($var, $field='') {
    if(empty($var) || !is_numeric($var)) {
      throw new Create_Exception(sprintf(__('%1$s (%2$s) must be a valid timestamp', 'wp-revenue-booster'),$field,$var));
    }
  }

  protected function validate_regex($pattern, $var, $field='') {
    if(!preg_match($pattern, $var)) {
      throw new Create_Exception(sprintf(__('%1$s (%2$s) must match the regex pattern: %3$s', 'wp-revenue-booster'),$field,$var,$pattern));
    }
  }
